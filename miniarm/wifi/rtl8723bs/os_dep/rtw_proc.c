...
int rtw_drv_proc_init(void)
{
	...
	DBG_871X_LEVEL(_drv_always_, "@rtw_drv_proc_init.start\n");
	...
	DBG_871X_LEVEL(_drv_always_, "@rtw_proc_create_dir=%x\n",(unsigned int)rtw_proc);
	...
	for (i = 0;i<drv_proc_hdls_num;i++) {
		...
		DBG_871X_LEVEL(_drv_always_, "@rtw_proc_create_entry=%x\n",(unsigned int)entry);
		if (!entry) {
			rtw_warn_on(1);
			goto exit;
		}
...
exit:
	DBG_871X_LEVEL(_drv_always_, "@rtw_drv_proc_init.return\n");
	return ret;
}
